...
dots.forEach((d, dIdx)=> d.addEventListener("click", () => changeSlideByIndex(dIdx+1)));
...
function changeSlideByIndex(n) {
    ...
    for (let i = 0; i < slides.length; i++) {
        ...
        !slides[i].className.includes("dui-util-disp-none") ? slides[i].className += " dui-util-disp-none" : null;
    }
    for (let i = 0; i < dots.length; i++) {
        dots[i].className = dots[i].className.replace(" slideshow-container__navigator--active", "");
    }
    // slides[slideIndex - 1].style.display = "block";
    slides[slideIndex - 1].className = slides[slideIndex - 1].className.replaceAll(" dui-util-disp-none", "");
